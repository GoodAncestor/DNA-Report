...
`python -m dnareport.benchmark preflight plan.json` verifies file hashes, sample columns, autosomal region/reference bounds, merged confident-region denominators, named difficult-region strata and a declared acceptance purpose. `score plan.json /new/evidence-directory --happy /installed/bin/hap.py` then runs the raw and filtered VCFs against the same fixed truth/mask/regions, with a prespecified haplotype-comparison engine and HG002 male ploidy. The template selects `vcfeval`, as recommended by the GIAB v5.0q release guidance; `xcmp` remains an explicit alternative for separately justified plans. It retains commands, logs, annotated tool outputs, hashes, the immutable plan copy and failure status. Default total comparison timeout is one hour and two threads.
...
The [GIAB v5.0q release guidance](https://ftp-trace.ncbi.nlm.nih.gov/ReferenceSamples/giab/release/AshkenazimTrio/HG002_NA24385_son/v5.0q/) recommends `--engine vcfeval`, `--gender male` and v3.6 stratifications. Comparator choice is pinned in the plan rather than silently hardcoded. The orchestration fixture verifies argument passing only; it does not establish installed vcfeval availability or scientific correctness. Reference compatibility and actual runtime execution remain pending.
