...
<div class="models"><section><h2>AlphaMissense</h2><p>Looks up a precomputed prediction for a protein-changing variant. A missing match does not mean harmless.</p></section><section><h2>AlphaGenome</h2><p>Predicts molecular effects from DNA sequence. Results depend on the scorer, tissue and model; they are not a personal disease probability.</p></section><section><h2>AlphaGenome Atlas / AVI</h2><p>Looks up precomputed variant impact rankings and available feature contributions for coding and non-coding changes. AVI combines AlphaGenome and AlphaMissense into one score, so agreement with AlphaMissense is not independent confirmation. AVI is an impact ranking, not a disease probability.</p></section></div>
<form id="explorer"><label for="variant">GRCh38 variant</label><input id="variant" name="variant" value="__VARIANT__" placeholder="22-36201698-A-C" required maxlength="125" autocomplete="off" spellcheck="false"><small>Chromosome-position-reference-alternate. Coordinates are 1-based. Use GRCh38 and verify the reference allele. Example: 22-36201698-A-C.</small><button type="submit">Look up evidence</button><button type="button" id="atlas">Look up Atlas / AVI</button><button type="button" id="predict">Request AlphaGenome prediction</button><small>Evidence lookup uses local databases and cached Atlas results when available. Atlas lookup sends the variant coordinates to Google’s Atlas service when enabled, to retrieve precomputed scores and feature contributions. Requesting a prediction sends this variant and its sequence context to Google’s AlphaGenome service when enabled. Results are research annotations; no sample genotype is inferred.</small></form>
...
async function run(predict,atlas=false){
 document.querySelectorAll('button').forEach(b=>b.disabled=true);message.textContent=predict?'Requesting a bounded prediction…':atlas?'Looking up Atlas / AVI…':'Looking up evidence…';frame.hidden=true;document.querySelector('#exports').hidden=true;
 try{const response=await fetch('/api/variant',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({variant:document.querySelector('#variant').value,reference_build:'GRCh38',predict,atlas})});const data=await response.json();if(!response.ok)throw new Error(typeof data.detail==='string'?data.detail:'The lookup could not complete. Please try again.');
...
def guided_demo_html(result):
    ...
    intro = '''<section class="ai-guide"><h2>Explore AI predictions</h2><p>Verified public examples show what each model contributes. AlphaMissense evaluates protein changes; AlphaGenome evaluates regulatory effects; AlphaGenome Atlas ranks predicted variant impact with AVI. The APOE example includes a verified Atlas AVI result alongside its AlphaMissense prediction. Compare the model evidence with the available ClinVar record, and open each variant in the explorer to investigate further.</p><p>AVI already includes AlphaMissense as one of its inputs, so the two agreeing is not independent confirmation.</p><p>These examples are separate variants, not a person’s genome. Model predictions support research and are not clinical conclusions. AlphaGenome has not been validated for, and is not approved for, any clinical use.</p><p><a href="/explore">Look up your own variant</a> · <a href="/demo/ai?format=json">Download JSON</a> · <a href="/demo/ai?format=markdown">Download Markdown</a></p></section>'''
    from biocore.licensing import commercial_mode
    if commercial_mode():
